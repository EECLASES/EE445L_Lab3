...
void Switch_Init(void){ 
  ...
  GPIO_PORTB_AMSEL_R &= ~0x20;      // 3) disable analog on PA5
  GPIO_PORTB_PCTL_R &= ~0x00F00000; // 4) PCTL GPIO on PA5
  GPIO_PORTB_DIR_R &= ~0x20;        // 5) direction PA5 input
  GPIO_PORTB_AFSEL_R &= ~0x20;      // 6) PA5 regular port function
  GPIO_PORTB_DEN_R |= 0x20;         // 7) enable PA5 digital port
}
//------------Switch_Input------------
...
uint32_t Switch_Input(void){
  return PB1; // return 0x20(pressed) or 0(not pressed)
}
uint32_t Switch_Input2(void){
  return (GPIO_PORTB_DATA_R&0x20); // 0x20(pressed) or 0(not pressed)
}
