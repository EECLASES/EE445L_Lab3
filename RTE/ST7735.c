...
int32_t Ymax,Ymin,X, Xmax, Xmin;        // X goes from 0 to 127
...
void ST7735_XYplot(uint32_t num, int32_t bufX[], 
  int32_t bufY[], uint16_t color){
		uint32_t i;
	for(i = 0; i < num; i++){
		if((bufY[i] >= Ymin) && (bufY[i] <= Ymax) && (bufX[i] >= Xmin ) && (bufX[i] <= Xmax)){
			
			// i goes from 0 to 127
			// x=MaxX maps to i=0
			// x=MaxX maps to i=127
			int32_t x = (127*(bufX[i]-Xmin))/(Xmax-Xmin);  
			// y=MaxY maps to j=32
			// y=MinY maps to j=159
			int32_t y = 32+(127*(Ymax-bufY[i]))/(Ymax-Ymin);
